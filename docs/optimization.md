...
**Especially relevant for Jerboa**: the compiler generates many structurally similar lambdas (match arms, method dispatchers, accessor functions). Setting this to 3-5 could significantly reduce code size and improve instruction cache utilization. Trade-off: can undo some cp0 inlining and complicate debugging.
...
## 16. Recommendations Specific to Jerboa
...
2. **`commonization-level` 3-5** — High value for Jerboa since the compiler generates many structurally similar lambdas from match, defmethod, and accessor compilation.

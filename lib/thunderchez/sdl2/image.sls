...
(library
  ...
  (import (chezscheme) 
	  (thunderchez ffi-utils)
	  (thunderchez sdl2)
	  (only (std srfi srfi-1) fold)
	  (only (thunderchez thunder-utils) string-replace string-split) 
	  (only (std srfi srfi-13) string-delete string-suffix? string-prefix?)
	  (std srfi srfi-14))

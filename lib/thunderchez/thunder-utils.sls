...
(library (thunderchez thunder-utils)
  ...
  (import (chezscheme)
	  (only (std srfi srfi-14) char-set? char-set-contains?)
	  (only (std srfi srfi-1) take drop))

  ;; Auxiliary keywords for optional/keys syntax
  (define-syntax /optional
    (lambda (x) (syntax-violation '/optional "misplaced use of auxiliary keyword" x)))
  (define-syntax /keys
    (lambda (x) (syntax-violation '/keys "misplaced use of auxiliary keyword" x)))

  ;; chop: split a flat list into sublists of size n
  ;; (chop '(a 1 b 2) 2) => ((a 1) (b 2))
  (define (chop lst n)
    (if (null? lst) '()
	(cons (take lst n)
	      (chop (drop lst n) n))))
  ...
  (define-syntax define/keys
	...
	[(_ (name params ... (/keys keys ...)) body ...)
	 (let ([keys-list
 	   ...
 	   #`(define name
	       (lambda (params ... . keys*)
		 (import (only (thunderchez thunder-utils) decode-keys chop))
